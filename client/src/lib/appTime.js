/**
 * App calendar timezone (default South Africa). Mirror server `src/lib/appTime.js`.
 * Optional: VITE_APP_TIMEZONE at build time to match APP_TIMEZONE on the API.
 */
...
export function getAppTimeZone() {
  try {
    const z = String(import.meta.env?.VITE_APP_TIMEZONE || '').trim();
    if (z) return z;
  } catch (_) {}
  return DEFAULT_TZ;
}
...
/** Number of days in a Gregorian month (monthIndex0 = 0..11). */
...
/** Column offset (0 = Sunday) for the first of month in the app zone. */
