...
export async function sendEmail({ to, subject, body, html = true, text, cc, attachments }) {
    try {
        ...
        const isGmail =
            ...
            emailHost.includes('googlemail.com') ||
            emailHost.includes('google.com');

        console.log('📧 EmailService: Using SMTP:', {
            host: emailHost,
            port: emailPort,
            provider: isOutlook ? 'outlook' : isGmail ? 'gmail' : 'custom',
            user: emailUser,
        ...
        // Gmail: port 587 + STARTTLS (same pattern as Outlook for reliable delivery)
        if (!emailSecure && emailPort === 587 && isGmail) {
            transportOptions.requireTLS = true;
            transportOptions.secure = false;
            transportOptions.tls = { rejectUnauthorized: false, minVersion: 'TLSv1.2' };
        }
            ...
            try {
                ...
                if (msg.includes('invalid login') || msg.includes('535') || msg.includes('authentication') || fullResponse.includes('535') || fullResponse.includes('5.7.139')) {
                    ...
                    if (isOutlook) {
                        ...
                        console.error('   • Gmail: Use an App Password (https://myaccount.google.com/apppasswords) with 2FA enabled; EMAIL_USER must be the full Gmail address.');
                    } else {
                        console.error('   • If using Gmail: App Password + 2FA; EMAIL_USER = full @gmail.com address.');
                    }
        ...
        const fromName = process.env.EMAIL_FROM_NAME || 'Simplyapp';
        const fromAddress = emailUser.includes('@')
            ? emailUser
            : (emailHost.includes('office365') || emailHost.includes('outlook')
                ? emailUser
                : `${emailUser}@gmail.com`);
        ...
        const textStr = (text != null ? String(text).trim() : '') || (looksLikeHtml ? 'Simplyapp – Access management – List distribution. Please find the attached documents.' : bodyStr);
        ...
        if (msg.includes('invalid login') || msg.includes('535') || msg.includes('authentication') || msg.includes('credentials')) {
            const helpfulError = new Error(
                'SMTP authentication failed. Check EMAIL_USER and EMAIL_PASS. Gmail: use an App Password with 2FA (https://myaccount.google.com/apppasswords). Microsoft 365: enable Authenticated SMTP if you see 535.7.139; App Password if MFA is on.'
            );
